    ...
    public static void main(String[] args) {
        ...
        while(true){
            try {
                ...
                System.out.println("Erro capturado -> Você digitou letra onde deveria ser número. Tente novamente!");
                scan.nextLine();
            }
        }
                ...
                switch (opcao) {
                    ...
                    case 2:
                        if(adotanteService.getAdotanteList().size() == 0) {
                            throw new MinhaExcecao("\nErro capturado -> Nenhum adotante cadastrado até o momento");
                        }
                    ...
                    case 4:
                        if(abrigo.getPetsDisponiveis().size() == 0) {
                            throw new MinhaExcecao("\nErro capturado -> Nenhum pet disponível para adoção no momento");
                        } else {
                    ...
                    case 5:
                        if(abrigo.getPetsDisponiveis().size() == 0) {
                            throw new MinhaExcecao("\nErro capturado -> Nenhum pet disponivel para adoção no momento");
                        }
                    ...
                    case 6:
                        if(abrigo.getAnimaisList().size() == 0) {
                            throw new MinhaExcecao("\nErro capturado -> Nenhum pet cadastrado no momento");
                        } else {
                        ...
                        if(adotanteService.getAdotanteList().size() == 0) {
                            ...
                            if(adotante != null) {
                                if(adotante.getPets().size() != 0) {
                                    throw new MinhaExcecao("Erro capturado -> Nossos termos de segurança não permite remover um adotante que já tenha adotado um pet.");
                                } else {
                                    adotanteService.getAdotanteList().remove(adotante);
                                    System.out.println("Adotante removido com sucesso!");
                                }
                            } else {
                                throw new MinhaExcecao("Erro capturado -> Adontate não encontrado no banco de dados.");
                            }
                        }
                        break;
